...
import { loadDaemonSettings, saveDaemonSettings, type DaemonSettings } from "./settings.js";
...
class HybridDaemon {
  ...
  private async start(status: "started" | "restarted"): Promise<JsonOutput> {
    const settings = loadDaemonSettings(this.paths.settingsFile);
  ...
  private async bootstrap(remoteControlEnabled: boolean): Promise<JsonOutput> {
    ...
    saveDaemonSettings(this.paths.settingsFile, settings);
  ...
  private async setRemoteControl(enabled: boolean): Promise<JsonOutput> {
    const previous = loadDaemonSettings(this.paths.settingsFile);
    const managed = reconcileManagedProcess(this.paths.pidFile);
    const managedOwnsSocket = managed ? this.ownsSocket(managed) : false;
...
export function runPidDaemonCommand(
  config: HybridConfig,
