...
- OpenAI Codex `0.145.0` is used and linked under Apache-2.0. The exact upstream revision is recorded in `config/compatibility.json`.
- Claude Agent SDK `0.3.219` is installed from Anthropic's npm package as an exact runtime dependency; CCodex does not copy or rehost its platform binaries. Use remains subject to Anthropic's applicable legal agreements.
